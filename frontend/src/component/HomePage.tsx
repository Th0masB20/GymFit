  ...
  return (
    <nav className="h-screen w-20 bg-main absolute top-0 flex flex-col justify-between items-center">
      <div className="flex justify-center bg-[rgba(255,255,255,0.95)] mt-4 p-1 rounded-lg hover:scale-110">
        <button className="bg-HomeImage bg-cover w-11 h-11" />
      </div>
      <div className="flex flex-col items-center justify-between h-48 w-full">
        <button className="bg-WorkoutImage bg-cover w-16 h-16 hover:scale-110" />
...
const MainBody = ({ user }: { user: IUser }): React.ReactElement => {
  if (user.workouts.length == 0) {
    return (
      <div className="absolute flex flex-col justify-center items-center w-80 h-48 bg-gradient-to-r from-main to-fourth rounded-lg top-80 left-1/2 -translate-x-1/2 -translate-y-1/2">
        <p className="text-center text-xl w-44">
          Start By Creating Your Workout Routine
        </p>

        <div className="w-8 h-8 bg-black rounded-full mt-3 hover:cursor-pointer hover:scale-110 flex justify-center items-center">
